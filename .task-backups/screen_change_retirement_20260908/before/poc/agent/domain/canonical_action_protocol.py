...
MODEL_STEP_DECISION_FIELDS = frozenset({"status", "action", "element_id", "source_element_id",
    ...
    "reason", "text", "app", "previous_action_outcome"})
...
def bind_same_response_action(payload: Mapping[str, Any], *, context: Any, observation: Any,
    ...
    if kind in MODEL_STEP_DIRECT_POINT_ACTIONS:
        ...
        if target["role"] == "input":
            element = _current_input_target(observation, context=context, target=target)
        ...
        element = _current_input_target(observation, context=context)
        params.update(_element_params(element))
...
def _current_input_target(observation: Any, *, context: Any,
    target: Mapping[str, Any] | None=None) -> UIElement:
