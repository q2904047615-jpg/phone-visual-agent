...
class CameraAlignmentFacts:
    ...
    def validate(self) -> None:
        ...
        reject_if(not isinstance(self.evidence, tuple), UISceneError("camera_alignment.evidence 必须是字符串元组。"))
        for item in self.evidence:
            reject_if(not isinstance(item, str) or not item.strip(), UISceneError('camera_alignment.evidence 只允许非空字符串。'))
...
class UIElement(ValidatedDataclassWire):
    ...
    def from_dict(cls, value: dict[str, Any], *, coordinate_scale: float=1.0) -> 'UIElement':
        ...
        element = cls(
            ...
            states=dict(states),
            evidence=tuple(item.strip() for item in evidence if isinstance(item, str) and item.strip()),
        )
        element.validate()
        return element
