...
from test_generic_scene_observer import audited_application_input, input_audit_payload, scene_payload
from test_qwen_visual_decision import patterned_frames, StatusOnlyProvider
from test_single_visual_loop import Adapter, LoopHarness, scene as loop_scene, decision as loop_decision
...
def wire(choice=None, audit=None):
    ...
    return {'protocol_version': SINGLE_STEP_OBSERVATION_PROTOCOL_VERSION,
        'coordinate_space': {'kind': 'axis_grid', 'width': 1000, 'height': 480},
...
def observe(payload, graph=None, *, allowed=None):
    ...
    frames = [frame.resize((240 if height == 480 else 720, height), Image.Resampling.NEAREST)
...
class PointSceneProjectionTests(unittest.TestCase):
    def test_saved_response_replays_through_observer_binder_controller(self):
        archived = json.loads((Path(__file__).parent / 'test_fixtures/point_scene_saved_response.json').read_text(encoding='utf-8'))
