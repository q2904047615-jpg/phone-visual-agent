...
from agent.domain.universal_action_controller import (
    ...
    UniversalActionController as _UniversalActionController,
    UniversalActionError,
)


class UniversalActionController(_UniversalActionController):
    """Upgrade only legacy test requests; production has no mechanical fallback."""

    def resolve_one(self, action, current, **kwargs):
        if action.action in {"input_verified_text", "clear_verified_text"} and "text_transport" not in action.params:
            target = current.get_element(action.params["element_id"])
            field_id = target.states.get("input_field_id", target.element_id)
            current = replace(current, elements=tuple(replace(item,
                states={"input_field_id": item.element_id, **item.states})
                if item.role == "input" else item for item in current.elements))
            prior = target.states.get("value", "")
            expected = action.params.get("text", "") if action.action == "input_verified_text" else ""
            params = {**action.params, "text_transport": "adb_keyboard", "input_field_id": field_id,
        ...
        return super().resolve_one(action, current, **kwargs)
