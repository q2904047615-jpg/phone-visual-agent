...
def execution_history_entry(*, step: int, requested_action: dict[str, Any],
    ...
    visual_outcome: Any = None, after_scene: str = "",
    execution_metadata: Mapping[str, Any] | None = None) -> dict[str, Any]:
    """Project executed facts without losing semantic targets or sharing mutable params."""
    metadata = execution_metadata if isinstance(execution_metadata, Mapping) else {}
    # Only explicit diagnostic strings cross into model history. Do not forward
    # commands/full receipts, infer acceptance, or rewrite Qwen's visual outcome.
    diagnostics = {key: metadata.get(key) if isinstance(metadata.get(key), str) else None
        for key in ('transport_status', 'transport_error')}
    return deepcopy({"step": step, "canonical_action": requested_action, "action": resolved_action,
        ...
        "visual_outcome": visual_outcome, "after_scene": after_scene, **diagnostics})
