...
function externalSession() {
  return {
    ...
    qwen_decision: { ...clone(qwenFixture.decision),
      reason: "当前页面显示向演示商户支付20元的确认按钮",
      next_action: { ...clone(qwenFixture.decision.next_action), params: {
        role: "button", target: "financial_transaction", label: "确认支付20元", tap_point: [0.5,0.6],
      }},
    },
    effect_confirmation_preview: { action: "tap_semantic", params: {
      role: "button", target: "financial_transaction", label: "确认支付20元", tap_point: [0.5,0.6],
    }},
    physical_actions: 0,
    evidence: [],
...
function externalActionSession() {
  ...
  session.confirmed_effect_ids = ["financial_transaction"];
  ...
  return http.createServer(async (request, response) => {
    ...
    if (url.pathname === "/api/device") {
      const registeredDevices = devices || [{
        ...
        capability_acceptance_actions: runtimeWire.capabilities,
      }];
      json(response, 200, {
    ...
    if (request.method === "POST" && url.pathname.endsWith("/auto")) {
      ...
      json(response, 200, {
        ...
        session: Object.values(runtimeWire).find(wire => wire.session &&
          url.pathname.includes(wire.session.session_id))?.resumed || afterActionSession(),
      });
      return;
    }
...
test("task budget fields submit configured limits", { timeout: 30000 }, async () => {
  ...
  try {
    ...
    assert.equal(requests.start.at(-1).max_observations, 456);
  } finally {
    await browser.close();
...
test("budget pause restores progress and resumes via the budget endpoint", { timeout: 30000 }, async () => {
  Object.values(requests).forEach(items => { items.length = 0; });
  const session = safeActionSessionForDevice("phone-01", "budget-paused-test");
  session.status = "budget_paused";
  session.confirmation_scope = null;
  session.confirmation_ready = false;
  session.qwen_decision = null;
  session.physical_actions = 100;
  session.execution_budget = { max_physical_actions: 100, max_observations: 200, observation_attempts: 150 };
  session.auto_pause_reason = "达到整任务设备动作预算，已暂停并保留进度。";
...
test("browser requests versioned task-status assets instead of stale cached URLs", { timeout: 30000 }, async () => {
  ...
  try {
    assert.ok(requests.assets.includes("/assets/app.js?v=20260907-runtime-contract-v1"));
...
test("current action approval executes exactly one bound login or payment action", { timeout: 30000 }, async () => {
  Object.values(requests).forEach(items => { items.length = 0; });
  const server = createServer();
  const { browser, page } = await launchFixturePage(server);
  ...
  try {
    ...
    assert.match(await page.locator("#riskWarning").innerText(), /当前选中动作摘要一致/);
    ...
    assert.match(await page.locator("#riskWarning").innerText(), /只执行该观察绑定的一次动作/);
...
for (const mode of ["paused", "budget_paused"]) {
  test("browser restores actual runtime/repository " + mode, {timeout: 30000}, async () => {
    Object.values(requests).forEach(items => { items.length = 0; });
    const wire = runtimeWire[mode];
    assert.equal(wire.active.length, 1);
    const session = wire.session;
    const server = createServer({
      devices: [{device_id: session.device_id, verified_actions: ["swipe"],
        capability_acceptance_actions: runtimeWire.capabilities}],
      activeSessions: wire.active, restoredSessions: {[session.session_id]: session},
    });
    ...
    try {
      ...
      assert.equal(requests.confirm.length, 0);
      assert.equal(wire.resumed.status, "succeeded");
      assert.equal(wire.resumed.physical_actions, 1);
    } finally {
      await browser.close();
      server.closeAllConnections();
      await new Promise(resolve => server.close(resolve));
