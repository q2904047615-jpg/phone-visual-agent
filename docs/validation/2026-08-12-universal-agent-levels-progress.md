...
- 修复多设备网页恢复边界：`/api/device` 的每个通用活动会话现在显式返回 `device_id`；网页只恢复
  ...
  机械臂调用。
- 修复多设备实时预览边界：JPEG/MJPEG 预览接口现在必须携带已登记 `device_id`，
  不再默认读取第一台设备。网页初始化和切换设备时均重建绑定该设备的预览 URL；
  缺少 ID 返回422，未登记 ID 返回404。
...
- 新增重启只读恢复。旧 trial 的报告和证据可继续查看，但动作 authority 和 promotion authority
  不跨进程恢复。
- 真机能力验收核心、API、网页控制台、多设备会话隔离和通用前进导航策略均已进入
  当前分支；具体真机能力仍只按本文记录的真实证据判定，不由代码存在自动推导。
...
只读取目标设备；不会把 phone-01 的任务、确认范围、轨迹或实时预览显示到 phone-02。
