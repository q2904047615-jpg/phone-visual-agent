...
首次执行时间：`2026-08-12T14:39:52+08:00`

安全审查修复后复跑时间：`2026-08-12T15:01:00+08:00`。复跑使用跨进程
设备租约检查，结果与首次一致：设备独占可用，但控制端和摄像头离线，
`physical_actions=0`。
...
完整离线回归另验证了跨进程设备租约、跨进程物理动作租约、断线后确认失效、
确认后目标语义与区域严格重绑定、以及多步证据不覆盖。最终 Python 测试
`541/541`、前端协议 `12/12`、浏览器契约 `3/3` 全部通过。跨进程租约
另使用真实子进程验证：持锁进程存活时第二进程不能取得同一设备/动作锁，
进程退出后锁由操作系统释放。
