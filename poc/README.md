...
设为 150%；由于 2560×1440 屏幕可能放不下被整体放大后的卖家控制窗口，建议为
`main.exe` 单独启用“由应用程序执行高 DPI 缩放”，让其他软件文字按 150% 显示、卖家控制
窗口保持完整原始尺寸。改变缩放后必须重启卖家软件和网页服务，先运行 `doctor`，再在安全
触点页做一次单点验收；不得直接在真实 App 中试点。
