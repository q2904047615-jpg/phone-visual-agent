...
当真机用户无法操作地址栏时，可以在启动测试服务器时用
`--root-action-mode input`、`--root-action-mode long_press` 或
`--root-action-mode drag`，让根页仅重定向到对应通用验收模式；手机仍须通过一次独立、
重新观察后的刷新动作加载新页面。该参数只选择无账号副作用的测试页，不执行机械臂动作。
