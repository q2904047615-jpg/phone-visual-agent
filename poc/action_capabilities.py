...
class CapabilityGap:
    ...
    def validate(self) -> None:
        ...
        if self.profile_digest and not re.fullmatch(r"[0-9a-f]{64}", self.profile_digest):
    ...
    def gap(
        ...
        *,
        ...
        return CapabilityGap(
            ...
            reason_code=str(
                self.actions[requested_action].get("gap_reason")
                or "device_capability_not_verified"
            ),
            supported_actions=self.supported_actions,
            required_parameters=tuple(str(item) for item in required_parameters),
            profile_digest=self.profile_digest,
        )


def build_device_capability_snapshot(
    *,
    device_id: str,
