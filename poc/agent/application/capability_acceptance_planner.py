...
_SEMANTIC_CONTRACTS: Final[dict[str, tuple[str, str]]] = {'tap_semantic': ('目标视觉控件触发后的页面状态可见', '新画面显示目标视觉控件预期的结构化状态变化'),
    'dismiss_overlay': ('当前遮挡层关闭后的底层页面可见', '新画面显示遮挡层消失且底层页面保持可见'),
    'scroll': ('当前页面沿目标方向移动后的新内容可见', '新画面显示与目标方向一致的结构化页面位移'),
    'swipe_element': ('当前目标元素按视觉轨迹移动或消失', '新画面证明目标元素已经按预期改变或消失'),
    'back': ('当前页面的上一级状态可见', '新画面显示可验证的上一级页面状态'), 'home': ('设备系统主屏幕可见', '新画面显示可验证的系统主屏幕状态'),
    'reveal_system_navigation': ('设备系统导航区域可见', '新画面显示结构化系统导航区域'), 'input_verified_text': ('目标输入区域显示指定的本地临时文字',
    '新画面逐字显示指定文字且没有提交或发送'), 'long_press': ('目标视觉对象显示持续触发后的可见状态', '新画面显示仅由持续触发产生的结构化状态变化'), 'drag': ('源视觉对象稳定到达目标视觉区域',
...
class CapabilityAcceptanceTaskGraphPlanner:
    ...
    def plan(self, raw_goal: str, *, device_id: str, task_id: str | None=None) -> DynamicTaskGraph:
        ...
        graph = DynamicTaskGraph(task_id=str(task_id or uuid.uuid4().hex), device_id=str(device_id or '').strip(),
            ...
            target_apps=(TargetApp(app_id='current_foreground_surface', app_name='当前前台手机界面'),),
            entities={'capability_trial_kind': self.candidate_action}), constraints=('仅允许当前本地验收页面内可由新画面验证的可逆变化',
            '不得提交、发送、删除、支付、修改账号或产生其他外部状态影响', '每次确认只允许一个物理动作且禁止自动重试'),
            completion_conditions=(CompletionCondition(condition_id='primitive_result_visible', description=completion,
            evidence_required=('动作后重新采集的可信画面', '结构化动作结果与页面变化证据')),), risk_actions=(),
            subgoals=(Subgoal(subgoal_id='certify_primitive', objective=objective, status='active', depends_on=(),
            constraints=('只使用当前可信画面中的唯一视觉对象', '动作后必须重新观察并验证结构化结果'), completion_conditions=(completion,),
