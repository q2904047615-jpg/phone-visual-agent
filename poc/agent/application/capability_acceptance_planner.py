...
_SEMANTIC_CONTRACTS: Final[dict[str, tuple[str, str]]] = {'tap_semantic': ('目标视觉控件触发后的页面状态可见', '新画面显示目标视觉控件预期的结构化状态变化'),
    'dismiss_overlay': ('当前遮挡层关闭后的底层页面可见', '新画面显示遮挡层消失且底层页面保持可见'), 'swipe': ('当前页面沿目标方向移动后的新内容可见',
    '新画面显示与目标方向一致的结构化页面位移'), 'back': ('当前页面的上一级状态可见', '新画面显示可验证的上一级页面状态'), 'home': ('设备系统主屏幕可见', '新画面显示可验证的系统主屏幕状态'),
