...
class UniversalAgentSessionApplicationService:
    ...
    def __init__(
        self,
        *,
        orchestrator_provider: Callable[[], UniversalAgentOrchestratorPort],
        sessions: AgentSessionRepository,
        ensure_device_ready: Callable[[str], None],
        exclusive_device_session: Callable[
            [str], AbstractContextManager[None]
        ],
        begin_new_task: Callable[[str], None],
    ) -> None:
    ...
    @staticmethod
    def _require_start_available(
        orchestrator: UniversalAgentOrchestratorPort,
        device_id: str,
    ) -> None:
        active_session_id = orchestrator.device_registry.active_session(device_id)
        if active_session_id is not None:
            raise AgentSessionConflictError(
                f"设备 {device_id} 已有活动任务：{active_session_id}。"
            )
    ...
    def start(
        self,
        command: StartUniversalAgentSessionCommand,
        ...
        if command.auto_advance and session.status in {
            "awaiting_confirmation",
            "needs_reobservation",
        }:
            with self._exclusive_device_session(command.device_id):
                automatic_progress = orchestrator.run_autonomous_safe_loop(
                    session,
                    max_physical_actions=12,
                    max_iterations=24,
                )
        return StartUniversalAgentSessionResult(
            session=session,
    ...
    def approve_effects(
        ...
        *,
        ...
        if confirmed is not True or confirmation is None:
            raise AgentSessionCommandError(
                "调用 Qwen 处理受限效果前必须确认完整效果作用域。"
            )
        require_session_device(
            session,
            str(confirmation.get("device_id") or ""),
        )
        orchestrator = self._orchestrator()
        ...
        return AgentSessionOperationResult(
            session=session,
            operation=result,
    ...
    def confirm(
        ...
        *,
        ...
        try:
            ...
            try:
                orchestrator.invalidate_confirmation(
                    session,
