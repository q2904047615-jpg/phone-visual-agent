...
def _newly_completed(
    previous: DynamicTaskGraph, revised: DynamicTaskGraph, *, existing_only: bool = False
) -> tuple[Any, ...]:
    ...
    return tuple(
        ...
        and (not existing_only or item.subgoal_id in old_by_id)
        and (
            old_by_id.get(item.subgoal_id) is None
...
class VerifiedAppSurfaceLineage:
    ...
    def matches_foreground(self, foreground_app_id: str) -> bool:
        ...
        return bool(foreground and foreground != "launcher") and foreground in {
            self.functional_foreground_app_id.strip().casefold(),
    ...
    def is_scoped_to(self, *, session: Any, graph: DynamicTaskGraph, physical_actions: int) -> bool:
        return (
            ...
            and self.task_id == graph.task_id
            and self.device_id == session.device_id == graph.device_id
            and self.physical_actions == physical_actions
        )
...
class AppSurfaceLineageAuthority:
    ...
    def transition_proves(
        *,
        ...
        if (
            any(value is None for value in (receipt, before_scene, after_scene, action, execution_result))
            or not session_id
            or str(getattr(completed_subgoal, "external_impact", "")) != "navigation_only"
        ...
        if active is None or active.subgoal_id != completed_subgoal.subgoal_id:
            return False
        actual_scope = (
            receipt.session_id, receipt.task_id, receipt.device_id, receipt.prior_revision,
            ...
            receipt.before_observation_id, receipt.before_fingerprint,
            receipt.after_observation_id, receipt.after_fingerprint,
        )
        expected_scope = (
            session_id, previous.task_id, previous.device_id, previous.revision,
            ...
            _action_digest(getattr(execution_result, "resolved_action", None)), "matched", 1,
        ...
        if (
            ...
            or str(getattr(before_scene, "foreground_app_id", "")).casefold() != "launcher"
        ...
        matches = tuple(
            element
            for element in tuple(getattr(before_scene, "elements", ()) or ())
        ...
        action_identity = tuple(
            str(value or "").strip()
            for value in (params.get("label"), params.get("role"), params.get("target") or params.get("meaning"))
        )
        element_identity = tuple(
            str(value or "").strip()
            for value in (getattr(element, "label", ""), getattr(element, "role", ""), getattr(element, "meaning", ""))
        )
        if action_identity != element_identity:
            return False
        action_terms = VisibleGoalEvidence.binding_terms(
            params.get("label"),
            params.get("target"),
            params.get("meaning"),
        ...
        bound_targets = tuple(
            ...
            if VisibleGoalEvidence.target_app_terms(app.app_id, app.app_name).intersection(action_terms)
        )
    ...
    def lineage_proves(
        *,
        ...
        return bool(
            ...
            and _is_descendant(previous, completed_subgoal, lineage.source_subgoal_id)
        )

    @classmethod
    def validate_newly_completed(
        cls,
        *,
        previous: DynamicTaskGraph,
        ...
        for item in _newly_completed(previous, revised):
            ...
            if (
                target_apps
                and not VisibleGoalEvidence.foreground_matches(scene, target_apps)
                and not cls.transition_proves(
                    previous=previous,
    ...
    def build(
        ...
        *,
        ...
        for item in _newly_completed(previous, revised, existing_only=True):
            ...
            target_apps = VisibleGoalEvidence.referenced_target_apps(previous, text, item.subgoal_id)
            if not target_apps or not cls.transition_proves(
                previous=previous,
                completed_subgoal=item,
                target_apps=target_apps,
                trusted_observation=trusted_observation,
            ...
            bound = [
                app for app in target_apps
                if VisibleGoalEvidence.target_app_terms(app.app_id, app.app_name).intersection(terms)
            ]
    ...
    def carry(
        *,
        ...
        if (
            lineage is None
            or scene is None
            or str(getattr(execution_result, "action_outcome", "")) not in POST_ACTION_OUTCOMES
            or not valid_delta
            or not lineage.is_scoped_to(session=session, graph=previous, physical_actions=prior_physical_actions)
            ...
            or not lineage.matches_foreground(str(getattr(scene, "foreground_app_id", "")))
        ):
            return None
        ...
        if current is None:
            return (
                replace(lineage, physical_actions=session.physical_actions) if revised.status == "completed" else None
            )
    ...
    def refresh(
        *,
        ...
        if (
            ...
            or not lineage.is_scoped_to(session=session, graph=graph, physical_actions=session.physical_actions)
            or not lineage.matches_foreground(str(getattr(prior_scene, "foreground_app_id", "")))
            or str(getattr(new_scene, "foreground_app_id", "")).casefold() == "launcher"
        ...
        def page_titles(scene: Any) -> frozenset[str]:
            return frozenset(
                str(getattr(element, "label", "") or "").strip().casefold()
                for element in tuple(getattr(scene, "elements", ()) or ())
                if str(getattr(element, "label", "") or "").strip()
                and (
                    str(getattr(element, "meaning", "") or "").casefold()
                    == "page_title"
                    or str(getattr(element, "meaning", "") or "")
                    .casefold()
                    .endswith(("_page_title", "_screen_title"))
                )
                and (getattr(element, "states", {}) or {}).get("fully_visible")
                is not False
        ...
        if (
            ...
            or current is None
            or not _is_descendant(graph, current, lineage.source_subgoal_id)
        ):
            return None
