...
class DeviceActionRequest:
    ...
    def validate(self) -> None:
        ...
        if (self.kind == 'long_press' and (isinstance(self.hold_seconds, bool) or not isinstance(self.hold_seconds,
            (int, float)) or (not 0.5 <= float(self.hold_seconds) <= 2.0))):
        ...
        if self.kind == 'clear_verified_text':
            ...
            if (isinstance(self.delete_count, bool) or not isinstance(self.delete_count,
                int) or (not 1 <= self.delete_count <= 100)):
                raise DeviceExecutionError("清空动作退格次数无效。")
        if (self.kind == 'wait_for_change' and (isinstance(self.wait_seconds, bool) or not isinstance(self.wait_seconds,
            (int, float)) or float(self.wait_seconds) < 0)):
            raise DeviceExecutionError("等待时长无效。")

    @staticmethod
    def _validate_point(value: tuple[int, int] | None, label: str) -> None:
        if (not isinstance(value, tuple) or len(value) != 2 or any((isinstance(part, bool) or not isinstance(part,
            int) or (not 0 <= part <= 1000) for part in value))):
