...
class DeviceActionRequest(DataclassWire):
    ...
    def validate(self) -> None:
        ...
        if self.kind in {'input_verified_text', 'clear_verified_text', 'press_enter'}:
            reject_if(self.text_transport != 'adb_keyboard' or self.text_scope is None,
                DeviceExecutionError("文字动作需要已配置的 ADB Keyboard 和授权 scope。"))
            reject_if(self.point is not None or self.end_point is not None,
                DeviceExecutionError("ADB Keyboard 文字动作不得携带机械落点。"))
            try:
                self.text_scope.validate()
            except ValueError as exc:
                raise DeviceExecutionError(f"ADB Keyboard scope 无效：{exc}") from exc
            if self.kind == 'clear_verified_text':
                reject_if(self.input_fragment not in (None, '')
                    or self.text_scope.fragment_text_digest != EMPTY_TEXT_DIGEST
                ...
                reject_if(not isinstance(self.input_fragment, str) or not self.input_fragment,
                    DeviceExecutionError("输入动作缺少确定性文字。"))
                reject_if(self.kind == 'press_enter' and self.input_fragment != "\n",
            ...
            reject_if(self.text_transport is not None or self.text_scope is not None
                or self.input_fragment is not None,
                DeviceExecutionError("非文字动作不得携带 text transport、正文或授权 scope。"))
