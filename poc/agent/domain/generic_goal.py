...
class VisibleGoalEvidence:
    ...
    def presence_only(cls, subgoal: Any) -> bool:
        conditions = tuple(filter(None, (str(item or "").strip() for item in getattr(subgoal, "completion_conditions", ()) or ())))
    ...
    def safe_element(
        ...
        *,
        ...
        return bool(
            ...
            and not cls.has_conflict(observation, str(getattr(item, "element_id", "") or ""))
        )
    ...
    def focused_input_fact(cls, observation: Any) -> str | None:
        ...
        candidates = [
            ...
            and cls.safe_element(item, observation, roles=frozenset({"input"}), goal_relevant=True)
        ]
        if len(candidates) != 1:
            return None
        item = candidates[0]
    ...
    def zero_action_fact(cls, subgoal: Any, observation: Any) -> str | None:
        ...
        return cls.focused_input_fact(observation) if conditions and all(focus.search(item) for item in conditions) else None

    @staticmethod
    def binding_terms(*values: Any) -> frozenset[str]:
        text = " ".join(str(value or "").casefold().replace("_", " ") for value in values)
        generic = {
            "action", "button", "control", "current", "display", "element", "foreground", "image", "item",
            "page", "screen", "show", "stable", "target", "view", "visible", "当前", "前台", "页面", "画面",
            "目标", "元素", "控件", "可见", "出现", "显示", "稳定", "完整", "唯一",
        }
        terms = {token for token in re.findall(r"[a-z0-9]{3,}", text) if token not in generic}
        for run in re.findall(r"[\u4e00-\u9fff]{2,}", text):
            terms.update(run[index:index + size] for size in range(2, min(6, len(run)) + 1) for index in range(len(run) - size + 1))
    ...
    def title_prefixes(*values: Any) -> tuple[str, ...]:
        ...
        return tuple(dict.fromkeys(match.group(1).strip().casefold() for pattern in patterns for match in pattern.finditer(text) if match.group(1).strip()))
    ...
    def surface_classes(cls, *values: Any) -> frozenset[str]:
        ...
        markers = {
            ...
            "title": ("标题", "题头", "title", "heading"), "list": ("列表", "清单", " list"),
            "input": ("输入框", "文本框", "input field", "textbox"), "menu": ("菜单", " menu"),
    ...
    def references_target_app(cls, text: str, target_app: Any) -> bool:
        ...
        identities = (
            cls.compact_app_phrase(getattr(target_app, "app_id", "")),
            cls.compact_app_phrase(getattr(target_app, "app_name", "")),
        )
        return bool(compact and any(identity in compact for identity in identities if identity and identity not in {"app", "application", "应用", "程序"}))

    @classmethod
    def names_only_target_app(cls, text: str, target_app: Any) -> bool:
        compact = cls.compact_app_phrase(text)
        identities = tuple(dict.fromkeys(identity for identity in (
            cls.compact_app_phrase(getattr(target_app, "app_id", "")),
            cls.compact_app_phrase(getattr(target_app, "app_name", "")),
        ) if identity and identity not in {"app", "application", "应用", "程序"}))
        ...
        generic = (
            "处于前台", "已经打开", "已经启动", "应用程序", "主界面", "主页面", "当前", "目标", "应用", "程序",
            "主页", "首页", "页面", "界面", "屏幕", "视图", "打开", "启动", "进入", "前台", "可见", "显示",
            "已经", "已", "在", "的", "并", "and", "application", "foreground", "launched", "opened", "visible",
            "current", "target", "screen", "interface", "page", "view", "home", "main", "launch", "open", "app", "is", "in", "the",
        )
    ...
    def subgoal_targets_launcher(graph: DynamicTaskGraph, subgoal_id: str) -> bool:
        ...
        return bool(subgoal is not None and surfaces.get(subgoal.surface_ref) is not None and surfaces[subgoal.surface_ref].kind == "launcher")
    ...
    def typed_system_surface_fact(graph: DynamicTaskGraph, subgoal_id: str, scene: Any) -> str | None:
        ...
        actions = {
            ...
            for ref in getattr(subgoal, "constraint_refs", ())
        ...
        if not action or expected_idempotent_system_surface_kind(action) != actual:
            return None
        return json.dumps({"action_kind": action, "operator": "equals", "predicate": "surface.kind", "source": "canonical_action_protocol", "value": actual}, ensure_ascii=False, sort_keys=True, separators=(",", ":"))

    @classmethod
    def referenced_target_apps(cls, graph: DynamicTaskGraph, text: str, subgoal_id: str = "") -> tuple[Any, ...]:
        if cls.subgoal_targets_launcher(graph, subgoal_id) or not cls.surface_classes(text).intersection({"page", "foreground_app"}):
            return ()
        return tuple(app for app in graph.goal.target_apps if str(app.app_id or "").strip().casefold() != "current_foreground" and cls.references_target_app(text, app))
    ...
    def page_identity_facts(scene: Any) -> tuple[str, ...]:
        values = [{
            ...
            "foreground_app_id": str(getattr(scene, "foreground_app_id", "") or ""),
        ...
        for item in tuple(getattr(scene, "elements", ()) or ()):
            ...
            if role in {"text", "container"} and (
                role == "container"
                or any(marker in meaning for marker in ("page", "screen", "view", "home", "title", "heading"))
            ):
    ...
    def intrinsic_surface_classes(cls, item: Any) -> frozenset[str]:
        ...
        mapped = {"input": "input", "textbox": "input", "text_input": "input", "list": "list", "list_item": "list", "menu": "menu", "menu_item": "menu", "dialog": "dialog", "modal": "dialog", "title": "title", "heading": "title"}.get(role)
        if mapped:
            classes.add(mapped)
        return frozenset(classes)
    ...
    def is_multi_text(text: str) -> bool:
        return bool(re.search(r"(?:和|与|及|同时|均|都|两者|两个|多个|分别|\bboth\b|\band\b|\ball\b|\btwo\b|\bmultiple\b)", str(text or "").casefold()))

    @classmethod
    def multi_candidates(cls, subgoal: Any, scene: Any, observation: Any) -> tuple[Any, ...] | None:
        text = " ".join(map(str, (getattr(subgoal, "objective", ""), *tuple(getattr(subgoal, "completion_conditions", ()) or ())))).casefold()
        terms = cls.binding_terms(text)
        if not cls.is_multi_text(text) or not terms:
            return None
        matched = [(item, cls.binding_terms(item.label, item.meaning, *item.evidence).intersection(terms)) for item in scene.elements]
        ...
        for index, pair in enumerate(matched):
            ...
            if not any(left.union(right).issubset(pair[1]) for left_index, left in enumerate(peers) for right in peers[left_index + 1:]):
                reduced.append(pair)
        reduced_terms = [item[1] for item in reduced]
        if not 2 <= len(reduced) <= 4 or any(not item.difference(frozenset().union(*(reduced_terms[:index] + reduced_terms[index + 1:]))) for index, item in enumerate(reduced_terms)):
            return None
    ...
    def visible_text_read(subgoal: Any) -> bool:
        if str(getattr(subgoal, "external_impact", "")) != "read_only":
            return False
        text = " ".join((str(getattr(subgoal, "objective", "") or ""), *tuple(getattr(subgoal, "completion_conditions", ()) or ()))).casefold()
        return bool(
            any(marker in text for marker in ("读取", "获取", "读出", "read", "report", "get the"))
            and any(marker in text for marker in ("标题", "题头", "错误提示", "错误信息", "状态提示", "title", "heading", "error message", "status message"))
            and not any(marker in text for marker in ("等于", "包含", "逐字", "指定文字", "是否为", "equals", "contains", "exactly", "whether"))
        )
    ...
    def unique_candidate(cls, scene: Any, observation: Any) -> Any | None:
        ...
        competing = any(item.element_id != candidate.element_id and item.states.get("goal_relevant") is True and float(item.confidence) >= MIN_TARGET_CONFIDENCE for item in getattr(scene, "elements", ()) or ())
    ...
    def evidence(cls, graph: DynamicTaskGraph, subgoal: Any, observation: Any) -> tuple[str, ...] | None:
        ...
        destination = subgoal.external_impact == "navigation_only" and bool(surfaces) and surfaces.issubset({"page", "destination", "foreground_app"})
        ...
        if subgoal.external_impact == "read_only" and not surfaces and not _named_visual_identity_anchor(conditions) and not cls.is_multi_text(text) and named_surface:
        ...
        if cls.is_multi_text(text):
            ...
            if candidate is not None:
                ...
                prefix_matches = bool(prefixes and all(candidate.label.casefold().startswith(item) for item in prefixes))
                ...
                element_surfaces = element_surfaces.difference({"title"}) if "title" in element_surfaces and prefix_matches else element_surfaces
                if not cls.safe_element(candidate, observation) or not terms or not (prefix_matches if prefixes else terms.intersection(candidate_terms.union(scene_terms))) or not element_surfaces.issubset(cls.intrinsic_surface_classes(candidate)):
                    return None
                ...
                scene_surfaces = scene_surfaces.union({"foreground_app"}) if app_matches else scene_surfaces
                element_surfaces = surfaces.difference(scene_surfaces)
                candidates = tuple(item for item in scene.elements if terms.intersection(cls.binding_terms(item.label, item.meaning, *item.evidence)) and element_surfaces.issubset(cls.intrinsic_surface_classes(item)))
                if not terms.intersection(scene_terms) or not 1 <= len(candidates) <= 4 or any(not cls.safe_element(item, observation) for item in candidates):
                    return None
        ...
        return (scene.summary, *((focus,) if focus else ()), *facts, *(fact for item in candidates for fact in item.evidence), *((*page_facts, typed_surface) if typed_surface else ()))
