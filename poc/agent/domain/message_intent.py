...
def subgoal_targets_recipient_control(recipient: str, *values: Any) -> bool:
    ...
    selection_markers = ('打开', '进入', '选择', '查找', '搜索', '定位', '匹配', 'open', 'enter', 'select', 'choose', 'find',
        'search', 'locate', 'match')
    non_selector_markers = ('输入', '草稿', '编辑', '发送', '消息正文', 'input', 'draft', 'edit', 'type', 'send', 'message body')
    return any((marker in visible for marker in selection_markers)) and (not any((marker in visible for marker
        in non_selector_markers)))
