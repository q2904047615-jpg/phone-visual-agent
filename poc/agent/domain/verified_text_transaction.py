...
def is_direct_latin_segment(value: Any) -> bool:
    ...
    return bool(isinstance(value, str) and 1 <= len(value) <= MAX_DIRECT_LATIN_SEGMENT_CHARS and all((char
        in DIRECT_LATIN_CHARACTERS for char in value)))
...
def next_keyboard_layout_towards(current_layout: Any, desired_layout: Any) -> str | None:
    ...
    if (current_layout not in KEYBOARD_LAYOUT_PATH or desired_layout not in KEYBOARD_LAYOUT_PATH
...
def required_keyboard_input_mode_for_step(step: VerifiedInputStep) -> str | None:
    ...
    if (step.kind == 'literal_key' and step.segment.isprintable()
    ...
    if _CHINESE_RE.fullmatch(first):
        ...
        for char in remaining:
            if not _CHINESE_RE.fullmatch(char) or len(segment) >= 4:
                break
            segment += char
        step = VerifiedInputStep(target_text=target, current_text=current_text, segment=segment, kind='chinese_pinyin',
            required_mode='chinese_pinyin', expected_value=current_text + segment, pinyin=local_pinyin(segment),
        ...
        for char in remaining:
            if char not in DIRECT_LATIN_CHARACTERS or len(segment) >= MAX_DIRECT_LATIN_SEGMENT_CHARS:
                break
            segment += char
        step = VerifiedInputStep(target_text=target, current_text=current_text, segment=segment, kind='direct_latin',
            required_mode='direct_latin', expected_value=current_text + segment, physical_keys=segment)
        ...
        for char in remaining:
            ...
            segment += char
        step = VerifiedInputStep(target_text=target, current_text=current_text, segment=segment, kind='direct_latin',
            required_mode='direct_latin', required_case_mode='upper', expected_value=current_text + segment,
            physical_keys=segment.casefold())
    else:
        segment = first
        step = VerifiedInputStep(target_text=target, current_text=current_text, segment=segment, kind='literal_key',
            required_mode='visible_key', expected_value=current_text + segment)
    step.validate()
    return step
