...
def _normalized_coverage_bounds(value: Any, *, label: str) -> list[float]:
    if (not isinstance(value, list) or len(value) != 4 or any((isinstance(item, bool) or not isinstance(item, (int,
    ...
    if (any((not 0.0 <= item <= 1.0 for item in bounds)) or bounds[2] - bounds[0] < MIN_COVERAGE_SPAN_X
...
def _validated_coverage(value: Any, *, label: str) -> list[float]:
    ...
    if (not isinstance(hull, list) or len(hull) < 3 or any((not isinstance(point,
    ...
    if any((abs(stored - computed) > 1e-06 for stored, computed in zip(bounds, derived))):
...
def validated_calibration_evidence(path: Path) -> dict[str, Any]:
    ...
    if (not isinstance(frame_size, list) or len(frame_size) != 2 or any((isinstance(item, bool) or not isinstance(item,
...
def _calibration_evidence(value: Any, *, action: str) -> dict[str, Any] | None:
    if action not in CALIBRATION_BOUND_ACTIONS:
        ...
        return None
    if (not isinstance(value, dict) or set(value) != {'version', 'sha256', 'frame_size', 'coverage_bounds',
    ...
    if (not isinstance(digest, str) or len(digest) != 64 or any((character not in '0123456789abcdef' for character
        in digest))):
...
def _normalized_point(value: Any, *, field: str) -> tuple[float, float] | None:
    if value is None:
        return None
    if (not isinstance(value, (list, tuple)) or len(value) != 2 or any((isinstance(item, bool) or not isinstance(item,
...
def _validate_frame_paths(value: Any, *, field: str, trial_root: Path, expected_sha256: Any) -> tuple[str, ...]:
    ...
    if (not isinstance(expected_sha256, list) or len(expected_sha256) != 4 or any((not isinstance(item,
...
def validate_acceptance_report(report_path: Path) -> dict[str, Any]:
    ...
    confirmation_scope = _confirmation_scope(report.get('confirmation_scope'), session_id=session_id, task_id=task_id,
        device_id=device_id)
    if (confirmation_scope['observation_id'] != before['observation_id']
    ...
    if (not orientation_credential.evidence_frame_fingerprint or orientation_credential.evidence_frame_fingerprint not
    ...
    if isinstance(raw_alignment, dict):
        ...
        if (compact_layout not in {'unknown', orientation_credential.camera_layout_orientation} or compact_rotation not
...
class PromotionAuthority:
    ...
    def _live_source(self) -> tuple[OrientationCredential, Any, Mapping[str, Any]]:
        if (self._source_nonce is None or self._orientation_credential is None or self._execution_result is None
    ...
    def validate_and_consume(self, value: Mapping[str, Any]) -> None:
        ...
        if (set(requested) != set(expected) or any((not isinstance(requested.get(key),
...
class CapabilityRegistryPromoter:
    ...
    def _promote_bound(self, report_path: Path, *, confirmation: Mapping[str, Any],
        ...
        try:
            ...
            if (report['trial_id'] != scope.trial_id or report['device_id'] != scope.device_id
