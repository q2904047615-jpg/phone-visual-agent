...
class DeviceControllerRegistry:
    ...
    def __init__(self, path: Path, *, promotable_actions: Collection[str], mock: bool=False) -> None:
        ...
        for raw in payload['devices']:
            ...
            if not device_id or device_id in self._controllers:
                raise DeviceControllerRegistryError('设备注册表存在空或重复的 device_id。')
    ...
    def provisional_controller(self, device_id: str, candidate_action: str) -> RobotController:
        ...
        if action not in self._promotable_actions:
            raise ProvisionalDeviceControllerError(f'动作 {action or 'missing'} 不能进入真机能力验收。')
        try:
        ...
        if action in original.verified_actions:
            raise ProvisionalDeviceControllerError(f'设备能力 {action} 已经通过真机验收。')
