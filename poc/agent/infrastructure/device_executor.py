...
class RobotDeviceExecutor:
    ...
    def _consume_click_receipt(self, *, expected_count: int) -> dict[str, Any]:
        ...
        raw = consumer()
        reject_if(
            not isinstance(raw, dict) or raw.get('input_events_dispatched') is not True or (raw.get('mechanical_contact_ack')
            is not False),
            DeviceExecutionError('机械控制端没有返回有效的单击事件派发凭据。', physical_actions=1),
        )
        click_count = raw.get("click_count", 1)
        reject_if(click_count != expected_count, DeviceExecutionError('点击派发回执的 click_count 与请求不一致。', physical_actions=1))
        return dict(raw)
    ...
    def _consume_swipe_receipt(self, *, expected_direction: str) -> dict[str, Any]:
        ...
        reject_if(not isinstance(raw, dict)
            ...
            or raw.get('input_events_dispatched') is not True
    ...
    def _long_press(self, request: DeviceActionRequest) -> DeviceExecutionResult:
        ...
        reject_if(not isinstance(raw, dict), DeviceExecutionError('机械控制端没有返回长按事件派发凭据。', physical_actions=1))
