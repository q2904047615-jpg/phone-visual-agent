...
class OrientationCredential:
    ...
    def validate(self) -> None:
        ...
        if (self.source not in {ORIENTATION_AUDIT_SOURCE, LOCAL_QWERTY_ORIENTATION_SOURCE,
        ...
        if not isinstance(self.evidence_frame_fingerprint, str) or len(self.evidence_frame_fingerprint) > 128:
        ...
        if (not isinstance(self.frame_size, tuple) or len(self.frame_size) != 2 or any((isinstance(v,
        ...
        if self.phone_content_rotation not in {'upright', 'rotated_90', 'rotated_180', 'rotated_270', 'unknown'}:
    ...
    def assert_authorizes(self, *, device_id: str, scene_fingerprint: str, frame_size: tuple[int, int],
        ...
        # an action scope.
        if self.phone_content_rotation != 'upright' and action not in FIXED_SYSTEM_NAVIGATION_ACTIONS:
    ...
    def claim_live_execution_source(self) -> None:
        ...
        with _AUDIT_SEAL_LOCK:
            if seal is None or _CLAIMED_AUDIT_CREDENTIALS.get(seal) is not self:
                raise OrientationSafetyError('方向凭据不是本进程已进入物理执行门的 live 对象。')
...
def _mint_locally_verified_qwerty_credential(*, device_id: str, scene_fingerprint: str, frame: Image.Image,
    ...
    for key in required:
        ...
        if (not isinstance(value, (list, tuple)) or len(value) != 2 or any((isinstance(part,
    ...
    if (not (points['q'][0] < points['p'][0] and points['a'][0] < points['l'][0]
...
def _mint_single_step_scene_credential(*, device_id: str, scene_fingerprint: str, frame: Image.Image,
    ...
    if camera_layout_orientation_value != local_layout:
        raise OrientationSafetyError('单步画面方向与本地稳定帧尺寸不一致。')
    seal = object()
    item = OrientationCredential(version=ORIENTATION_CREDENTIAL_VERSION, credential_id=uuid.uuid4().hex,
...
def _claim_audit_seal(credential: OrientationCredential) -> _FrameVisualBinding:
    ...
    with _AUDIT_SEAL_LOCK:
        if seal is None or seal not in _LIVE_AUDIT_SEALS:
            raise OrientationSafetyError('方向凭据不是本进程实际独立审计直接签发，或已使用。')
