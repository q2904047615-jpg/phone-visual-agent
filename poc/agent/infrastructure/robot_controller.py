...
def qwerty_keyboard_config_from_anchors(anchors: dict[str, Any], *, key_hold: float=0.18,
    ...
    return {'source': 'vision_anchors_normalized', 'rows': [{'keys': 'qwertyuiop', 'x_start': q_x / 1000.0,
        ...
        'x_step': middle_step / 1000.0, 'y': middle_y / 1000.0}, {'keys': 'zxcvbnm', 'x_start': z_x / 1000.0,
        'x_step': bottom_step / 1000.0, 'y': bottom_y / 1000.0}], 'backspace_x_ratio': backspace_x / 1000.0,
        'backspace_y_ratio': backspace_y / 1000.0, 'key_hold': max(0.08, min(float(key_hold), 0.5)),
        'inter_key_wait': max(0.05, min(float(inter_key_wait), 0.5))}
...
class RobotController:
    ...
    def hardware_capabilities(self) -> dict[str, bool]:
        return {action: action in self.verified_actions for action in ('tap_semantic', 'dismiss_overlay', 'swipe',
            'back', 'home', 'open_recent_apps', 'wait_for_change', 'input_verified_text', 'double_tap', 'long_press',
    ...
    def hardware_capability_profile(self) -> dict[str, Any]:
        ...
        actions['input_verified_text']['text'] = {'canonical_max_chars': 4000,
            ...
            'non_lowercase_transport': 'audited_visible_key_sequence', 'max_fields': 32, 'max_targets': 32,
            'segments': ['direct_latin', 'chinese_pinyin', 'visible_literal_key'],
            'newline': 'visible_multiline_enter_key_only', 'unsupported_character_policy': 'structured_capability_gap'}
        actions['clear_verified_text'] = {**actions['input_verified_text'],
            'enabled': bool(enabled.get('input_verified_text')), 'layouts': ['qwerty', 'numeric', 'symbol',
            'generic_visible_backspace'], 'verified_delete_count': {'min': 1, 'max': 100}}
        ...
        actions['press_enter'] = {'enabled': bool(enabled.get('tap_semantic')), 'primitive': 'vision_tap_relative',
            'requires': ['fresh_visible_enter_key', 'key_action_newline', 'active_multiline_input_field',
    ...
    def validate_verified_text(self, text: str, input_states: dict[str, Any], *, target_text: str | None=None,
        input_method: str | None=None, pinyin: str | None=None) -> None:
    ...
    def vision_type_pinyin(self, text: str, pinyin: str, keyboard_layout: dict[str, Any] | None=None) -> None:
        ...
        for (index, key) in enumerate(pinyin):
            ...
            seller_gui.click_client_point(hwnd, point[0], point[1], countdown=0,
...
class MockRobotController(RobotController):
    ...
    def __init__(self, *, verified_actions: set[str] | frozenset[str] | None=None,
        ...
        all_actions = {'tap_semantic', 'dismiss_overlay', 'swipe', 'back', 'home', 'open_recent_apps',
            'wait_for_change', 'input_verified_text', 'double_tap', 'long_press', 'drag', 'reveal_system_navigation'}
