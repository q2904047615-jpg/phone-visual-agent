...
class CanonicalActionProtocolTests(unittest.TestCase):
    ...
    def test_active_clear_accepts_bound_preedit_when_app_value_is_empty(self) -> None:
        semantic_ir = input_ir(active="type_last_char")
        semantic_ir = replace(
            semantic_ir,
            constraints=(
        ...
        expectations = report.candidates[0].transition.expectations
        self.assertEqual(
            [
                ("element.state.value", "equals", ""),
                ("element.state.ime_preedit_text", "absent", None),
            ],
            [
                (item.predicate, item.operator, item.value)
    ...
    def test_active_exact_input_exposes_clear_for_conflicting_preedit(self) -> None:
        semantic_ir = input_ir(active="type_last_char")
        current_scene = scene(
            element(
                "input",
