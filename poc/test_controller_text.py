...
class UISceneTests(unittest.TestCase):
    ...
    def test_verified_input_uses_unique_typed_field_identity_after_layout_shift(self) -> None:
        ...
        # A fresh post-action frame may omit input_structure; Qwen's same-frame
        # postcondition then remains authoritative instead of a local veto.
        self.assertEqual(
            ("控制器确认 typed 输入值：'agent'",),
            UniversalActionController().verify_after_action(
                resolved,
                before,
                replace(after, elements=()),
            ),
        )
