...
from agent.infrastructure import device_runtime_resources as resource_module
import web_app
...
class DeviceRuntimeResourceRegistryTests(unittest.TestCase):
    def test_repeated_lookup_constructs_each_resource_only_once_per_device(self) -> None:
        with patch.object(resource_module.threading, 'Lock', wraps=threading.Lock) as locks, \
            patch.object(resource_module, 'DeviceCameraCoordinator', wraps=DeviceCameraCoordinator) as cameras:
            registry = DeviceRuntimeResourceRegistry(('phone-a',))
            for device in ('phone-a', 'phone-b'):
                for _ in range(20):
                    registry.coordination_lock(device)
                    registry.camera_coordinator(device)
            self.assertEqual(2, locks.call_count)
            self.assertEqual(2, cameras.call_count)

    def test_concurrent_first_camera_lookup_preserves_one_coordinator(self) -> None:
        ...
        def resolve() -> None:
            ...
            results.append(registry.camera_coordinator('phone-parallel'))
