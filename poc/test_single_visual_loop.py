...
class WholeTaskLoopTests(LoopHarness):
    ...
    def test_effect_approval_can_be_consumed_without_executing_action(self):
        loop,s,a=self.start([(scene(0),decision('tap_semantic',meaning='authentication')),
            (scene(1),decision())],goal='登录账号')
        self.assertEqual(s.status,'awaiting_effect_confirmation')
        loop.approve_effects_without_execution(s,s.effect_confirmation_authority.scope())
        self.assertEqual(s.status,'awaiting_confirmation')
        self.assertEqual(s.physical_actions,0)
        self.assertEqual(a.calls,[])
        loop.confirm_one(s,s.confirmation_authority.scope())
        self.assertEqual(s.physical_actions,1)
        self.assertEqual(len(a.calls),1)
