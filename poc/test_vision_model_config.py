...
class VisionModelConfigTests(unittest.TestCase):
    def test_default_is_qwen3_7_plus_non_thinking(self) -> None:
...
class DashScopeVisionModelRequestTests(unittest.TestCase):
    ...
    def test_strict_json_schema_disables_thinking_even_when_provider_enables_it(self) -> None:
        provider = DashScopeVisionProvider(api_key="test-key", enable_thinking=True, max_attempts=1)
        response_format = {
            "type": "json_schema",
            "json_schema": {
                "name": "scene",
        ...
        with patch(
            ...
            provider._chat([{"role": "user", "content": "JSON"}], max_tokens=None,
                           response_format=response_format)
        self.assertFalse(mocked.call_args.kwargs["json"]["enable_thinking"])
