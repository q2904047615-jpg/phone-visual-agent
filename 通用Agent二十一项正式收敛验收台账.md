...
## 43. 输入结构严格 schema 与模型输出预算不匹配

### 43.1 验收台账与根因证据

- 微信真机会话 `fd9650dc63f84354a54ca9859e14244b` 已连续完成 Home、进入微信、进入文件传输
  助手三个 matched 物理动作；第四步发现输入框残留 `codex`，正确提出 `clear_verified_text`，但动作前
  独立输入结构审计在 `0` 物理动作时失败。
- `qwen_visual_revision_7_..._confirmation_qwen_failure.json`、
  `refresh_step_4_b9aafceb_frame_attempt_1_qwen_failure.json` 和 attempt 2 的响应都被分类为
  `truncated_json`；原始长度约 2650--2680 字符，分别截断在 `case_switch` 或 `layout_switches` 对象中间。
- 当前严格 parser 允许最多 4 个输入框、4 个预编辑区、每区 8 个候选、键盘锚点、模式键、退格键、
  大小写键、8 个字面键和 4 个布局键，但该独立审计预算只有 `1000` tokens。主要根因是允许的正式
  schema 容量与调用预算不一致，不是微信页面、中文目标或清空动作的专用问题。
- 同一原始响应还按 prompt 合法返回了输入框右侧、与输入框分离的 `right_button=发送`。parser 目前
  只在该相邻按钮字段不完整时丢弃它，字段完整时反而要求按钮位于输入框内部并否决整个输入框；这是
  可选非授权结构与输入目标绑定过紧的第二处通用合同不一致。

### 43.2 同类样本、通用修复与边界

- 将输入结构审计的独立预算提高到能覆盖当前完整严格结构的保守值，并在 prompt 中要求单行紧凑 JSON；
  不改字段集合、数组上限、置信度、唯一输入框、键盘状态、几何复核或动作授权规则。
- `right_button` 明确位于输入框外、保持足够垂直对齐时，无论可选对象字段完整或只是合法子集，都只
  丢弃这个不授予权限的相邻按钮并保留输入框原始 bounds；按钮位于输入框内时仍按现有严格合同裁切，
  额外字段、无效 bounds、重叠歧义和不对齐结构仍失败关闭。
- 保留截断失败关闭：响应仍不完整时不得补全、不得猜测、不得执行动作；同一会话已连续失败两次，
  不再调用模型或真机，修复后必须从新会话重新建立观察和 authority。
- 用包含输入框、多个候选、完整 QWERTY 锚点、模式/退格/大小写/布局键的长结构做变化样本；同时保留
  缺字段、额外字段、越界数组和 malformed JSON 反测，证明扩大的是容量而不是协议权限。

### 43.3 验证与停止条件

- 先运行输入审计预算/完整长结构正测和截断反测，再运行 observer 相关回归与一次完整 Python 回归；
  全绿后本地提交并只重载项目 Uvicorn。
- 真机只创建一个全新微信会话，从当前页面先回 Home 再重新进入；任一物理动作不匹配或同类模型格式
  失败再次出现即停止，不复用 `fd9650dc63f84354a54ca9859e14244b`。
...
## 46. 条件清空在非空观察后被具体化为独立节点

### 46.1 验收台账与根因证据

- 新会话 `b6a8ea09522d454d80162c999361e98d` 从 Launcher 点击唯一“微信”入口并 matched，动作后四帧
  可信观察已到达文件传输助手，且唯一目标输入框为 `value=codex`、`focused=true`、
  `fully_visible=true`、`direct_latin`。
- DeepSeek 根据“当前确实非空”将用户的“如果已有内容则清空”具体化为 `clear_input`；
