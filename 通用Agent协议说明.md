...
“发送、发布、点赞、评论、关注、收藏、订阅、支付、购买、删除、提交、保存、邀请、加入、确认、同意、授权”等语义会被本地提升为外部影响，不能伪装成普通导航。
...
发送、提交、删除、关注、评论、支付等会改变账号或外部世界的动作，仍需一次明确确认。这次确认绑定：session、task、device、revision、subgoal、risk IDs、具体收件人/正文或操作意图摘要。确认后最多立即执行一个与该风险草稿绑定的动作，不再出现“先确认风险、再确认同一个发送动作”的双重确认。

例如“打开微信，进入文件传输助手，输入你好并发送”：打开、导航、聚焦和已验证输入属于可自动推进部分；真正点击发送前暂停一次，展示具体会话和正文，确认后只允许一次发送，随后重新观察发送结果。

`unknown` 只允许 0 动作观察和重新分类；不能通过确认直接升级成物理权限。
